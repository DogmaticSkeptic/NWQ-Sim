...
int main(int argc, char* argv[]) {
    ...
    int minN     = std::stoi(argv[3]);
    int maxN     = std::stoi(argv[4]);
    int step     = std::stoi(argv[5]);
    std::string filename = argv[6];
    std::ofstream ofs(filename);
    ofs << "#bond_dim parallel_time[s] sequential_time[s]\n";

    for(int N = minN; N <= maxN; N += step) {
        std::vector<size_t> tasks(ntasks, static_cast<size_t>(N));
        ...
        ac.allocate(0);
        int64_t next = -1;
        if(ec_par.pg().rank().value() == 0) next = ac.fetch_add(0,1);
        ec_par.pg().broadcast(&next,0);
